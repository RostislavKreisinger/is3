...
//            vd(this);
//            vd($(this));
//            if($(this).data('search') === true){settings.search = true;}
//            if($(this).data('max') !== undefined ){settings.maxSelected = $(this).data('max');}
            if (this.sumo || !$(this).is('select')) { vd("sumo initializes yet;"); return this.sumo; } //already initialized 
            // vd(settings);
            this.sumo = {
                ...
                search:function(){
                    ...
                    // O.chkAll = $('<i>');
                    O.search = $('<p class="search"><input class="search-input"></input></p>');
                    O.search.find('.search-input').on('keyup', function(){
                        var input = $(this);
                        if(input.val() === '' || input.val() === undefined) {
                            O.optDiv.find('ul.options li.hidden').removeClass('hidden')
                        }
                        clearTimeout(O.searchTimeout);
                        O.searchTimeout = setTimeout(function(O){
                            O.optDiv.find('ul.options li').each(function(ix,e){
                                e = $(e);
                                if (e.find('label').text().includes(input.val())) {
...
//                                O.hideOpts();
                ...
                onOptClick: function (li) {
                    ...
                    li.click(function () {
                        ...
                        if( settings.disableEmptySelect && O.E.val() == li.attr('data-val') ){
                        ...
                        if(settings.maxSelected !== false){
                            if( O.getSelected().length >= settings.maxSelected ){
                                O.disableNotSelectedItems()
                            }else{
                                O.enableAllItem();
                            }
                        }
                ...
                vRange: function (i) {
                    ...
                    if (opts.length <= i || i < 0) throw "index '" + i + "' out of bounds"
                    return O;
                },
                ...
                toggSel: function (c, i) {
                    ...
                    if(settings.maxSelected !== false){
                        if( this.getSelected().length >= settings.maxSelected ){
                            this.disableNotSelectedItems()
                        }else{
                            this.enableAllItem();
                        }
                    }
                ...
                */
                ...
                add: function (val, txt, i, selected) {
                    ...
                    if (typeof i == "undefined" || i == null || opts.length == i) { // add it to the last if given index is last no or no index provides.
                        O.E.append(opt);
                ...
                removeAll: function () {
                    var O = this;
                    for(var k = O.E.children('option').length-1; k >= 0; k--){
                        this.remove(k);
                    }
                },
                ...
                getSelected: function(){
                    ...
                    for(var k = 0; k < O.E.children('option').length; k++){
                        if (O.E.children('option')[k].selected) {
                            selected.push(O.E.children('option')[k].value);
                        }
                    }
                    return selected;
                },
                
                disableNotSelectedItems: function(){
                    var O = this;
                    for(var k = 0; k < O.E.children('option').length; k++){
                        if (!O.E.children('option')[k].selected) {
                            this.disableItem(k);
                ...
                enableAllItem: function () { 
                    var O = this;
                    for(var k = 0; k < O.E.children('option').length; k++){
                        if (O.E.children('option')[k].disabled) {
                            this.enableItem(k);
                        }
                    }
                },
